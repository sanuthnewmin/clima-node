...
# Enable CORS for all origins (handles localhost and Railway domains)
CORS(app, resources={
    r"/chat": {
        "origins": ["*"],
...
# Frontend serving routes for single-service deployment
@app.route('/')
def serve_index():
    """Serve the React frontend index.html"""
    frontend_path = os.path.join(os.path.dirname(os.path.dirname(__file__)), 'clima-node', 'dist', 'index.html')
    if os.path.exists(frontend_path):
        return send_from_directory(os.path.join(os.path.dirname(os.path.dirname(__file__)), 'clima-node', 'dist'), 'index.html')
    return jsonify({'message': 'ClimaNode API Running', 'status': 'healthy'}), 200

@app.route('/assets/<path:path>')
def serve_assets(path):
    """Serve static assets from React build"""
    return send_from_directory(os.path.join(os.path.dirname(os.path.dirname(__file__)), 'clima-node', 'dist', 'assets'), path)

@app.route('/<path:path>')
def serve_react_app(path):
    """Serve React app for client-side routing"""
    # Check if the requested path is an API route
    if path.startswith('api/') or path in ['chat', 'sensor-data', 'dashboard-data', 'sensor-summary']:
    ...
    if os.path.exists(frontend_path):
        return send_from_directory(os.path.join(os.path.dirname(os.path.dirname(__file__)), 'clima-node', 'dist'), 'index.html')
    return jsonify({'message': 'ClimaNode API Running'}), 200
